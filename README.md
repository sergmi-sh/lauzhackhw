# ДЗ по основам глубинного обучения

Реализована LCCN с STFT фронтендом с delta и delta-delta коэффициентами.

Предполагается, что датасет распакован в директорию data (то есть там лежит data/LA/LA/...), которая сама находится на директорию выше чекаута проекта (то есть проект и data лежат в одной директории).

Модель обучается train.py и сохраняет лучший чекпоинт (по EER) в saved/lightcnn-fixed/model_best.pth.

Запускается inference.py оттуда же, сохраняет результаты в data/saved/lightcnn-fixed-results/results.csv.

Немного изменены тренировщики и датасеты шаблона, чтобы генерировать итоговые результаты (метаданные из датасета отправляются в тренировщик, который сохраняет результаты, которые потом пишутся в results.csv).

Немного изменен механизм метрик, чтобы правильно считать EER, т.к. его нельзя считать по батчам.

Добавлен транфсорм паддинга и обрезки, метрика EER, датасет ASVspoof, модель и фронтенд, и всё это вместе с тренировочным рецептом
сконфигурировано новыми файлами.
